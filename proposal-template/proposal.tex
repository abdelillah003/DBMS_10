...
\begin{figure}[ht]
  ...
  \includegraphics[width=0.85\linewidth]{ERdraft.png}
...
the current bookings. Additional forms allow the user to create new bookings,
inspect booking details and manage reservations. The frontend communicates
with the FastAPI backend using REST requests.
...
\begin{minipage}[t]{0.47\linewidth}\centering
  ...
  \captionof{figure}{Main screen displaying current bookings retrieved using \texttt{GET /bookings}.}
  \label{fig:fe-main}
\end{minipage}
...
\end{minipage}\hfill
\begin{minipage}[t]{0.47\linewidth}\centering
  \includegraphics[width=\linewidth]{Detail.png}
  \captionof{figure}{Booking details screen displaying reservation information and associated services.}
  \label{fig:fe-second}
\end{minipage}
\end{figure}
...
current bookings retrieved from the backend using the
\texttt{GET /bookings} endpoint.

Figure~\ref{fig:fe-detail} presents the booking form, which is used to create
new reservations by sending data to the
\texttt{POST /bookings} endpoint.

Finally, Figure~\ref{fig:fe-second} shows the booking details screen, where
users can inspect reservation information and the associated hotel services.
...
The project consists of six database tables and five REST API endpoints.
The database manages guests, rooms, room categories, bookings, hotel services
and the relationship between bookings and services. The API supports reading and creating booking information.
